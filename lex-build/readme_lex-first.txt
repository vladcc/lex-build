...
Each stage of the generation is a separate awk script. All stages share the
common lex_lib.awk library. E.g. if you'd like to generate a lexer in C from a
description in src.txt:

$ awk -f lex-first.awk -f lex_lib.awk src.txt | awk -f lex-c.awk -f lex_lib.awk
